		  ...
		  <div class="container-fluid">
		    ...
		    <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
	          ...
	          <form name="formSearch" id="search" class="navbar-form navbar-left" action="goSearch">
					...
					<div class="form-group">
						<input class="btn btn-success" type="submit" value="千寻一下" name="submit"/>
					</div>	
	          </form>
...
<div id="1" class="contents">
				...
				<div class="panel panel-primary">
				   ...
				   <div class="panel-body">
					   <table class="table table-bordered">
					      <th>LinkLoveZONE</th><th>管理</th>
					      <c:forEach items="${allLink}" var="alk">
						      <tr class="warning">
						      	<td><a href="http://<c:out value="${alk.linkLoveURL}"/>"><c:out value="${alk.linkName}"/></a></td>
						      	<td><a class="btn btn-default" href="manage?go=link&id=<c:out value="${alk.linkId}"/>">删除</a></td>
						      </tr>
					      </c:forEach>
					   </table>
				    </div>
			  ...
			  <tbody>
				  ...
				  <c:forEach items="${leaveEach}" var="leave">
					  <tr>
					    ...
					    <td><a class="btn btn-default" href="manage?go=leave&id=<c:out value="${leave.leaveId}"/>">删除</a></td>
					  </tr>
				 </c:forEach>
				</tbody>
			</table>
  	...
  	<section class="col-md-8">
  		...
  		<c:forEach items="${allPic}" var="picget">
	  		<div class="col-md-3 imgshow">
	  			<div><img alt="" src="<c:out value="${picget.src}"/>"></div>
	  			<div><a class="btn btn-default" href="manage?go=pic&id=<c:out value="${picget.photoId}"/>">删除</a></div>
	  		</div>
  		</c:forEach>
  	</section>
  	<!--上传图片-->
  	<section class="col-md-4">
  		<li class="list-group-item active"><h4 class="list-group-item-heading">上传图片</h4></li>
  		 ...
  		 <c:forEach items="${articleEach}" var="ate">
	  		<div class="col-md-12 one">
	  			...
	  			<div class="col-md-12 article">
	  				<div  class="navabar-default"><c:out value="${ate.content}"/></div>
	  			</div>
	  			<div>
	  				<span><a class="btn btn-default" href="manage?go=article&id=<c:out value="${ate.articleId}"/>">删除</a></span>
	  				<span><a onclick="changeArticle();" class="btn btn-default">修改</a></span>
	  			</div>
	  		</div>
  		  </c:forEach>
  ...
  <script type="text/javascript">
  		  	function changeArticle(){
  		   		var uu = UE.getEditor('container');
  		   		var uu.setContent("Hello");
  		   	}
  		  </script>
<script>
...
<script type="text/javascript" src="${ctx}/front/js/my.js"></script>
</body>
</html>